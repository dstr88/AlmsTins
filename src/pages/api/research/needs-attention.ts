...
export const GET: APIRoute = async ({ request }) => {
	...
	try {
		const t1 = Date.now();
		const cached = await withTimeout(getCache<object>(`t:${tenantId}:research:needs-attention:v2`), 5_000, 'turso-cache');
	...
	const t2 = Date.now();
	let unmatchedResult: any, suggestedResult: any, resolvedResult: any;
	try {
		[unmatchedResult, suggestedResult, resolvedResult] = await withTimeout(
			Promise.all([
		...
		return new Response(JSON.stringify({ error: String(err), unmatched: [], suggested: [], resolved: [], symbols: [], total: 0, unmatchedCapped: false }), {
			status: 200,
			headers: { 'Content-Type': 'application/json' },
		});
	}
	...
	void setCache(`t:${tenantId}:research:needs-attention:v2`, payload, CACHE_TTL);
