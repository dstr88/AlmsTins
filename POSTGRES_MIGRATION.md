...
- [~] **4 RLS** — BUILT (not yet applied). Policy SQL for 74 tables (`src/scripts/sql/20260618_rls_policies.sql`, generated by `genRlsPolicies.mjs` w/ coverage check); shim plumbing (`dbContext.ts` AsyncLocalStorage + `db.pg.ts` two-pool owner-bypass / web-role `SET LOCAL app.tenant_id`+`app.user_id`, inert until `WEB_DATABASE_URL`); middleware establishes context per request; app-level filters kept. Proof `pgRlsProof.mjs` = 10/10 (tenant_id / user_id / FK-subquery all isolate + fail-closed + WITH CHECK, on real PG, rolled back). Remaining: create non-owner web role + grants + `WEB_DATABASE_URL` (Render, your step), then apply the policy SQL to PG.
